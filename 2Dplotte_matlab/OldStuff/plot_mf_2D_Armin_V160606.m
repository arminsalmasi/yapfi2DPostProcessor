...
selectedTime = [300, 900, 3600, 4600, 7200];
ndtList = zeros(size(selectedTime, 2),2);
for i = 1: size(selectedTime,2)
    [m, idx] = min(abs(TIME - selectedTime(i)));
    ndtList(i, :) = [idx, TIME(idx)];
end
...
elementNameList = [' C'; 'Co'; ' N'; 'Ti'; ' W'];

for idx = 1:size(ndtList, 1)
    for plot_element=1:NUMBER_OF_ELEMENTS
        x=zeros(1,ngp(1));
        y=zeros(1,ngp(2));
        k=1;
        m=1;
        ...
        for plot_timestep=1:ndtList(idx, 1)
        ...
        elementName = elementNameList(plot_element, :);
        ndtValue = ndtList(idx, 2);
        save(['data_result_ndt-' int2str(ndtValue) '_plot-element_' int2str(plot_element) '.mat'], ...
            'ndtValue', 'plot_element', 'elementName', 'x', 'y', 'xf_final');
    end
end
%figure
