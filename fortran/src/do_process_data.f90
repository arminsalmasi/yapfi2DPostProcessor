...
!-------------------------------------------------------------------------------------------------------------------------------------
  function do_process_data(root_tmp)
    ...
    double precision :: sum_x_substitutional, temp_cx, temp_cy, xcc, ycc
...
!        goto 10
    ...
    do t = 1 , root_tmp%processed%number_of_timesteps
      ...
      allocate(root_tmp%processed%timesteps(t)%node(root_tmp%processed%number_of_gridpoints(1),root_tmp%processed%number_of_gridpoints(2),root_tmp%processed%number_of_gridpoints(3)))
      ...
      do nx=1, root_tmp%processed%number_of_gridpoints(1)                   
        ...
        do ny=1, root_tmp%processed%number_of_gridpoints(2)
          do nz=1, root_tmp%processed%number_of_gridpoints(3)
            root_tmp%processed%timesteps(t)%node(nx,ny,nz)%vertex_index = vc
            root_tmp%processed%timesteps(t)%node(nx,ny,nz)%xyz_indexes(:) = (/nx,ny,nz/)
            SELECT CASE (root_tmp%processed%number_of_dimensions)
!              CASE (1)
!                ! root_tmp%processed%timesteps(t)%node(nx,ny,nz)%xyz_coordinates(:) = (/root_tmp%unprocessed%fvcc(cc), 0.0D0, 0.0D0/)                                                                           !adjust_number_of_coordinates_with_dimensionality
!                ! cc= cc+cc_interval
              CASE (2)
                root_tmp%processed%timesteps(t)%node(nx,ny,nz)%xyz_coordinates(:) = (/root_tmp%unprocessed%fvcc(cc), root_tmp%unprocessed%fvcc(cc+1), 0.0D0/)                                                   !adjust_number_of_coordinates_with_dimensio
                xcc =  ((root_tmp%processed%timesteps(t)%node(nx,ny,nz)%xyz_coordinates(1)-temp_cx)*2)
                ycc =  ((root_tmp%processed%timesteps(t)%node(nx,ny,nz)%xyz_coordinates(2)-temp_cy)*2)
                root_tmp%processed%timesteps(t)%node(nx,ny,nz)%vm=(xcc*2)*(ycc*2)      
                temp_cy= temp_cy+ (ycc)
                cc= cc+cc_interval                                                                                                                                                                                        !adjust_number_of_coordinates_with_dimensionality
!              CASE (3)
!                !    root_tmp%processed%timesteps(t)%node(nx,ny,nz)%xyz_coordinates(:) = (/root_tmp%unprocessed%fvcc(cc), root_tmp%unprocessed%fvcc(cc+1), root_tmp%unprocessed%fvcc(cc+2)/)                           !adjust_number_of_coordinates_with_dimens
!                !    cc= cc+cc_interval                                                                                                                                                                                        !adjust_number_of_coordinates_with_dimensionality
!              CASE DEFAULT
!                errorflag = .true.
!                goto 10
            ...
            allocate(root_tmp%processed%timesteps(t)%node(nx,ny,nz)%mole_fractions(root_tmp%processed%number_of_elements))
            allocate(root_tmp%processed%timesteps(t)%node(nx,ny,nz)%chemical_potentials(root_tmp%processed%number_of_elements))
            allocate(root_tmp%processed%timesteps(t)%node(nx,ny,nz)%phase_fractions(root_tmp%processed%number_of_phase))
              
            !prob_elements    
            do i = 1, root_tmp%processed%number_of_elements 
              root_tmp%processed%timesteps(t)%node(nx,ny,nz)%mole_fractions(i) = root_tmp%unprocessed%molfrc(e) 
              root_tmp%processed%timesteps(t)%node(nx,ny,nz)%chemical_potentials(i) = root_tmp%unprocessed%chpot(e) 
              e=e+1
            enddo
              
            !prob_phases      
            do i = 1, root_tmp%processed%number_of_phase 
              root_tmp%processed%timesteps(t)%node(nx,ny,nz)%phase_fractions(i) = root_tmp%unprocessed%phsfrc(p) 
              p=p+1                                                                    
            enddo  
            vc= vc+1
          enddo               
