...
!-------------------------------------------------------------------------------------------------------------------------------------
  function do_process_data(root_tmp)
    ...
    root_tmp%processed%current_region%number_of_gridpoints = root_tmp%unprocessed%ngrd    !int
             

    SELECT CASE (root_tmp%processed%current_region%number_of_dimensions)
!      CASE (1)
!      root_tmp%processed%current_region%number_of_gridpoints(2:3) = 1                                                 !DIMENSIONALITY_ERROR_HANDLER
...
!                ! cc= cc+cc_interval
              CASE (2)
                root_tmp%processed%current_region%timesteps(t)%nodes(nx,ny,nz)%xyz_coordinates(:) = (/root_tmp%unprocessed%fvcc(cc), root_tmp%unprocessed%fvcc(cc+1), 0.0D0/)                                                   !adjust_number_of_coordinates_with_dimensio
...
!              CASE (3)
!                !    root_tmp%processed%current_region%timesteps(t)%nodes(nx,ny,nz)%xyz_coordinates(:) = (/root_tmp%unprocessed%fvcc(cc), root_tmp%unprocessed%fvcc(cc+1), root_tmp%unprocessed%fvcc(cc+2)/)                           !adjust_number_of_coordinates_with_dimens
!                !    cc= cc+cc_interval                                                                                                                                                                                        !adjust_number_of_coordinates_with_dimensionality
!              CASE DEFAULT
!                errorflag = .true.
!                goto 10
            ...
            !prob_phases      
            do i = 1, root_tmp%processed%current_region%number_of_phase 
